...
const getCatInfo = () => {
    ...
    return  {name, age, color}     //Equivalente a {name: name, age: age, color: color}
  }
  
  //destruction assignment
  const {name, age, color} = getCatInfo()

  const message = `${name} é um gato ${color} de ${age} anos.`
  //const message = `${getCatInfo().name} é um gato ${getCatInfo().color} de ${getCatInfo().age} anos.`
  ...
  */
  ...
  const external = () => {                        //Não funciona por causa do escopo léxico
  ...
  */
  ...
  const zequinha = dogs.find(dog => dog.name === 'Zequinha') //Retorna a primeira ocorrencia
