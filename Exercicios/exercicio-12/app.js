...
//   totalPrice += prices[i]
// }
totalPrice = prices.reduce(getTotalPrices, 0)
...
const myFunc = (arg1, arg2, arg3) => {
    ...
    const isSomeArgsUndefined = [arg1, arg2, arg3].includes(undefined)
    const isOk = 'A função foi invocada com 3 argumentos'
    const isNotOk = 'A função deve ser invocada com 3 argumentos'
    return isSomeArgsUndefined ? isNotOk : isOk
}
...
const getPluralOrSingular = (quantily, singular, plural) => {
  return quantily === 1 ? singular : plural
}

const getAvaliabeSpacesMassage = (spaces, booksIn) => {
...
booksBox.addBooks = booksQuantily => {
  ...
  const boxSpaceAreNotEnough = booksBox.booksIn + booksQuantily  > spaces
  if(boxSpaceAreNotEnough){
    return getAvaliabeSpacesMassage(spaces, booksBox.booksIn)
  }
  
  booksBox.booksIn += booksQuantily
